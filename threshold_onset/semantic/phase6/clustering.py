...
def calculate_distance(vec1: Dict[str, float], vec2: Dict[str, float]) -> float:
    """
    Calculate Euclidean distance between two consequence vectors.

    Uses the intersection of keys present in both vectors (no process-global
    key cache — mixed schemas in one run, or tests with different key sets,
    would otherwise corrupt distances).
    ...
    Returns:
        Euclidean distance
    """
    keys = frozenset(vec1.keys()) & frozenset(vec2.keys())
    if not keys:
        return float('inf')

    squared_diff = sum((vec1.get(k, 0.0) - vec2.get(k, 0.0)) ** 2 for k in keys)
...
def compute_intra_cluster_distance(
    ...
    For large clusters, uses random sampling (up to max_pairs pairs) instead
    of full O(n²) enumeration.  This is a mechanical approximation — the
    sample set is deterministic (sorted order) so results are reproducible.
...
def compute_cluster_stability(
    ...
    rng = _random.Random(seed)  # isolated RNG — no global state mutation
...
# Cache: (n_vectors, k_min, k_max, seed) -> optimal_k
# Avoids rerunning expensive PAM + bootstrap when same input arrives twice
# (e.g. during paradigm validation repeated runs).
_SELECT_K_CACHE: dict = {}
...
def select_optimal_k(
    ...
    Results are cached by (n_vectors, k_min, resolved_k_max, seed) to avoid
    redundant PAM + bootstrap iterations on repeated calls.
    ...
    cache_key = (n, k_min, k_max, seed)
    if cache_key in _SELECT_K_CACHE:
        logger.info(f"select_optimal_k: cache hit for n={n} k=[{k_min},{k_max}]")
        return _SELECT_K_CACHE[cache_key]
    ...
    _SELECT_K_CACHE[cache_key] = best_k
    return best_k
