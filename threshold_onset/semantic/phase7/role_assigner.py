...
def _build_topology_cache(continuation_observer: Any):
    """
    Build incoming_map and direct_edges from the adjacency graph once.

    Returns:
        (incoming_map, direct_edges) — caller passes these to
        compute_binder_properties to avoid rebuilding per cluster.
    """
    ...
    return incoming_map, direct_edges
...
def compute_binder_properties(
    ...
    incoming_map: Optional[Dict[str, Set[str]]] = None,
    direct_edges: Optional[Set[tuple]] = None,
) -> Dict[str, float]:
    """
    ...
    Args:
        ...
        incoming_map: Pre-built incoming links (built once for all clusters)
        direct_edges: Pre-built direct-edge set (built once for all clusters)
    ...
    adjacency = continuation_observer.adjacency
    # Build topology structures only when not supplied by caller.
    if incoming_map is None or direct_edges is None:
        incoming_map, direct_edges = _build_topology_cache(continuation_observer)
...
def assign_roles_from_properties(
    ...
    Returns:
        (cluster_id -> role name, cluster_id -> property aggregates used for assignment)
    """
    ...
    # Return both roles and cluster_properties so callers can avoid
    # recomputing cluster properties (e.g. in RoleEmergenceEngine.emerge).
    return roles, cluster_properties
