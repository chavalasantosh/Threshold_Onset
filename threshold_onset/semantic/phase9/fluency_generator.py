...
class FluencyGenerator:
    ...
    def build_experience_table(
        ...
        for (source_hash, target_hash), delta in edge_deltas.items():
            ...
            # ConsequenceDelta is a TypedDict (dict at runtime); use .get() with fallback
            # to stay safe if a custom ConsequenceDelta subclass or dataclass is used.
            if isinstance(delta, dict):
                survival_delta = delta.get('survival_delta', 0.0)
            else:
                survival_delta = getattr(delta, 'survival_delta', 0.0)
    ...
    def score_transition(
        ...
        _stability_cache: Optional[Dict] = None,
    ) -> float:
        """
        ...
        Args:
            ...
            _stability_cache: Optional dict for memoising stability lookups
                              across multiple score_transition calls in one
                              generate() invocation.
        ...
        # Component 1: Stability score (cached to avoid repeated consequence field queries)
        cache_key = (source_hash, target_hash)
        if _stability_cache is not None and cache_key in _stability_cache:
            stability = _stability_cache[cache_key]
        else:
            stability = calculate_stability_score(
                cache_key,
                self.consequence_field
            )
            if _stability_cache is not None:
                _stability_cache[cache_key] = stability
    ...
    def generate(
        ...
        # Per-generate-call stability cache: avoids re-querying consequence field
        # for the same (source_hash, target_hash) pair at different steps.
        _stability_cache: Dict[Tuple[str, str], float] = {}
        # Build isolated RNG once (no global random.seed() inside the loop)
        import random as _rnd_mod
        _rng = _rnd_mod.Random(seed) if seed is not None else _rnd_mod
        ...
        for step in range(length - 1):
            ...
            for target_hash in allowed_targets:
                ...
                # Score transition (pass stability cache for this generate() call)
                score = self.score_transition(
                    transition,
                    current_role_sequence,
                    sequence[-self.novelty_window:],
                    _stability_cache=_stability_cache,
                )
            ...
            # Deterministic tie-breaking using isolated RNG (no global state mutation)
            if seed is not None:
                next_symbol = _rng.choice(candidates)
            else:
