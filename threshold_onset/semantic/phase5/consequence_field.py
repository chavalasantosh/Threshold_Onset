...
from threshold_onset.semantic.common.utils import (
    ...
    calculate_entropy_from_options,
)
...
class ConsequenceFieldEngine:
    ...
    def __init__(
        ...
        phase10_metrics: Optional[Dict[str, Any]] = None,
    ):
        """
        ...
        Args:
            ...
            phase10_metrics: Optional Phase 10 JSON snapshot (directed continuation);
                stored for provenance in ``save()`` metadata; does not alter rollouts.
        """
        ...
        self.phase10_metrics = phase10_metrics if isinstance(phase10_metrics, dict) else None
        ...
        # Cache for computed vectors — lock guards against duplicate work under ThreadPoolExecutor
    ...
    def build(
        ...
        # Compute consequence vectors in parallel using stdlib ThreadPoolExecutor.
        # Rollouts are CPU-light (pure-Python dict traversal), so threads work well.
        _max_workers = min(len(identity_hashes), max(1, (os.cpu_count() or 2) - 1))

        def _compute_one(ih):
            return ih, self.compute_consequence_vector(ih, seed=seed)

        identity_vectors = {}
        if len(identity_hashes) > 1 and _max_workers > 1:
            with concurrent.futures.ThreadPoolExecutor(max_workers=_max_workers,
                                         thread_name_prefix="phase5-vec") as _pool:
                futs = {_pool.submit(_compute_one, ih): ih for ih in identity_hashes}
                for i, fut in enumerate(concurrent.futures.as_completed(futs)):
                    try:
                        ih, vec = fut.result()
                        identity_vectors[ih] = vec
                        if (i + 1) % 10 == 0:
                            logger.info(f"Computed {i + 1}/{len(identity_hashes)} vectors")
                    except Exception as e:
                        ih = futs[fut]
                        logger.error(f"Failed to compute vector for {ih}: {e}")
                        raise ConsequenceFieldError(
                            f"Failed to compute vector for {ih}: {e}",
                            details={'identity_hash': ih}
                        ) from e
        else:
            for i, identity_hash in enumerate(identity_hashes):
                try:
                    _, vec = _compute_one(identity_hash)
                    identity_vectors[identity_hash] = vec
                    if (i + 1) % 10 == 0:
                        logger.info(f"Computed {i + 1}/{len(identity_hashes)} vectors")
                except Exception as e:
        ...
        edge_deltas = {}
        total_edges = 0  # initialise before branch so metadata block always has it
        ...
        if self.continuation_observer is None or not hasattr(self.continuation_observer, 'adjacency') or self.continuation_observer.adjacency is None:
            ...
            # Collect all (source, target) pairs to compute deltas for
            edge_pairs = [
                (src, tgt)
                for src in identity_hashes
                for tgt in self.continuation_observer.adjacency.get(src, set())
            ]

            def _compute_edge_delta_task(pair):
                src, tgt = pair
                return pair, self.compute_counterfactual_edge_delta(src, tgt, seed=seed)

            # Parallelize edge delta computation across all CPU cores.
            _ew = min(len(edge_pairs), max(1, (os.cpu_count() or 2) - 1))
            if len(edge_pairs) > 1 and _ew > 1:
                with concurrent.futures.ThreadPoolExecutor(
                    max_workers=_ew, thread_name_prefix="phase5-delta"
                ) as pool:
                    futs = {pool.submit(_compute_edge_delta_task, p): p for p in edge_pairs}
                    done = 0
                    for fut in concurrent.futures.as_completed(futs):
                        try:
                            (src, tgt), delta = fut.result()
                            edge_deltas[(src, tgt)] = delta
                        except Exception as e:
                            logger.warning(f"Failed to compute delta for {futs[fut]}: {e}")
                        done += 1
                        if done % 50 == 0:
                            logger.info(f"Computed {done}/{total_edges} edge deltas")
            else:
                for done, pair in enumerate(edge_pairs, 1):
                    try:
                        _, delta = _compute_edge_delta_task(pair)
                        edge_deltas[pair] = delta
                    except Exception as e:
                        logger.warning(f"Failed to compute delta for {pair}: {e}")
                    if done % 50 == 0:
                        logger.info(f"Computed {done}/{total_edges} edge deltas")
    ...
    def compute_consequence_vector(
        ...
        # Thread-safe cache check-before-compute
        with self._cache_lock:
            cached = self._identity_vectors_cache.get(identity_hash)
        ...
        # Resolve adjacency once and reuse for out_degree + dead_end_risk below.
        _adjacency = (
            self.continuation_observer.adjacency
            if self.continuation_observer
               and hasattr(self.continuation_observer, 'adjacency')
               and self.continuation_observer.adjacency
            else {}
        )
        outgoing_set = _adjacency.get(identity_hash, set())
        out_degree = len(outgoing_set)
        ...
        # dead_end_risk — reuse outgoing_set and _adjacency already resolved above
        dead_end_count = sum(
            1 for target in outgoing_set
            if len(_adjacency.get(target, set())) == 0
        )
    ...
    def save(self, filepath: str) -> None:
        ...
        data = {
            ...
            'metadata': {
                ...
                'policies': self.policies,
                **(
                    {'phase10_metrics': dict(self.phase10_metrics)}
                    if self.phase10_metrics
                    else {}
                ),
            }
        }
