...
class TestRoleAssigner(unittest.TestCase):
    ...
    def test_assign_roles_from_properties(self):
        ...
        roles, _props = assign_roles_from_properties(
            clusters,
            identity_vectors,
            percentile_high=75,
            percentile_low=25
        ...
        # Check role is one of valid roles (ordinal fallback may use *_candidate)
        valid_roles = {
            'anchor', 'driver', 'gate', 'binder', 'terminator', 'unclassified',
            'anchor_candidate', 'driver_candidate', 'terminator_candidate',
        }
