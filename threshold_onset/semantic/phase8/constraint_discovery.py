...
class ConstraintDiscoveryEngine:
    ...
    def __init__(
        ...
        # Optional: for small inputs, lower min_frequency so patterns can appear.
        # Off by default so defaults stay stable for tests and explicit configs.
        if phase8_config.get('adaptive_min_frequency', False) and self.symbol_sequences:
            total_symbols = sum(len(seq) for seq in self.symbol_sequences if seq)
            if total_symbols < 20:
                self.min_frequency = 1
            elif total_symbols < 50:
                self.min_frequency = min(self.min_frequency, 2)
    ...
    def save(self, filepath: str) -> None:
        ...
        # Use '|'-separated keys to avoid collisions if role names ever contain commas.
        data = {
            'role_patterns': {
