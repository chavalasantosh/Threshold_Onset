...
def cluster_residues(residues, threshold=CLUSTER_THRESHOLD, return_members=False):
    """
    ...
    Args:
        ...
        return_members: if True, also return the actual cluster member lists

    Returns:
        Dictionary with:
        - 'cluster_count': number of clusters (int)
        - 'cluster_sizes': list of cluster sizes (unordered, no distribution interpretation)
        - 'cluster_members': list of cluster lists (only when return_members=True)
    """
    if not residues:
        result = {'cluster_count': 0, 'cluster_sizes': []}
        if return_members:
    ...
    if return_members:
        result['cluster_members'] = clusters
    return result
