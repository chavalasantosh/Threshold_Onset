...
class CorpusState:
    ...
    def update(
        ...
        with self._lock:
            ...
            # identity_hashes is already a set (type annotation: Set[str]).
            # Convert only if needed; avoid double-set creation.
            identity_set = identity_hashes if isinstance(identity_hashes, set) else set(identity_hashes)
            d_core = self.decay_rate * self.core_decay_factor
            for h in self.identity_stability:
                if h not in identity_set:
            ...
            # 4. Decay absent edges — iterate once, skip present ones
            for key in self.edge_weights:
                if key not in canonical_edges:
                    self.edge_weights[key] *= 1.0 - self.decay_rate

            # 5. Atomicize — only check reinforced identities (they are the only ones
            # whose stability could have newly crossed T_atomic this update).
            for h in identity_set:
                if self.identity_stability.get(h, 0.0) > self.T_atomic:
