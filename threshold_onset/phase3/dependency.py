...
def _map_residues_to_identities(residues, phase2_metrics, segment_window):
    ...
    # Single-pass: compute hash once per segment, check both dicts simultaneously.
    # _hash_segment is lru_cache'd, so repeated segments across calls are free.
    for i in range(len(residues) - segment_window + 1):
        segment = tuple(residues[i:i + segment_window])
        seg_hash = _hash_segment(segment)
        ...
        if repeatable_hashes and seg_hash in repeatable_hashes:
            for residue_idx in range(i, i + segment_window):
                residue_to_identity.setdefault(residue_idx, set()).add(seg_hash)
