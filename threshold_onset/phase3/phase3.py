...
from threshold_onset.phase3.relation import extract_relations
from threshold_onset.phase3.graph import build_graph
from threshold_onset.phase3.interaction import detect_interactions
from threshold_onset.phase3.dependency import measure_dependencies
from threshold_onset.phase3.influence import measure_influence
from threshold_onset.phase3.persistence import measure_relation_persistence
from threshold_onset.phase3.stability import measure_relation_stability
...
def _compute_path_lengths(nodes, edges):
    """
    Compute shortest path lengths between all reachable node pairs using BFS.

    Optimised: builds adjacency list once, then runs one BFS per source node.
    Distances between each (source, target) pair are recorded only once
    (source < target in sorted order) to halve the work.
    ...
    for h1, h2 in edges:
        if h1 in adjacency and h2 in adjacency:
    ...
    for idx, start_node in enumerate(node_list):
        # Only visit nodes that appear later in the sorted list to avoid
        # recording each pair twice (start→end and end→start are the same length).
        remaining = set(node_list[idx + 1:])
        if not remaining:
            break
