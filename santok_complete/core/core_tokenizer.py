...
try:
    ...
    def _lru_cache(maxsize=128):  # type: ignore[misc]
        def decorator(fn):
            return fn
        return decorator
...
def _is_unicode_letter_or_mark(ch):
    """Tamil, Telugu, etc. (stdlib unicodedata); falls back to Indic blocks if needed."""
    if _unicodedata is not None:
        ...
        return False
    c = ord(ch)
    if (0x0B80 <= c <= 0x0BFF) or (0x0C00 <= c <= 0x0C7F):
        return True
    if _is_cjk(ch) or _is_devanagari(ch) or _is_arabic(ch) or _is_cyrillic(ch):
        return True
    return False


def _is_word_char(ch):
    return _is_alpha(ch) or _is_digit(ch) or _is_unicode_letter_or_mark(ch)
...
def _bpe_like_split(word):
    ...
    # Check longer patterns before shorter ones — prevents "th" consuming the "t"
    # in "the" before the trigram gets a chance to match.
    _THREE = frozenset(["the", "and", "ing", "ion", "tio", "ent", "for", "ter",
                        "hat", "tha", "ere", "ate", "his", "con", "res", "ver",
                        "all", "ons", "nce", "men"])
    _TWO = frozenset(["th", "he", "in", "er", "an", "re", "ed", "nd", "on",
                      "en", "at", "ou", "it", "is", "or", "ti", "as", "to",
                      "nt", "ng"])
    result = []
    i = 0
    n = len(word)
    ...
    while i < n:
        # 3-char check FIRST so trigrams like "the" are not consumed as "th" + "e"
        if i + 3 <= n and word[i:i+3] in _THREE:
            result.append(word[i:i+3])
            i += 3
        elif i + 2 <= n and word[i:i+2] in _TWO:
...
def _syllable_split(word):
    ...
    for ch in word:
        ...
        if ch in vowels:
            syllables.append(''.join(current_parts))
            current_parts = []
...
def tokenize_text(text, tokenizer_type="word", language=None, use_parallel=False, source_tag=None, **kwargs):
    ...
    # Route to appropriate tokenizer
        ...
        # Default to "fixed" — "simple" was an invalid alias that silently fell through
        strategy = kwargs.get("strategy", "fixed")
        if strategy == "simple":
            strategy = "fixed"
        tokens = tokenize_subword(text, kwargs.get("max_length", 3), strategy)
...
def _tokenize_large_text(text, tokenizer_type, **kwargs):
    ...
    for i in range(0, len(text), chunk_size):
        ...
        # Adjust token IDs to maintain uniqueness (safe: not all token formats have 'id')
        for token in chunk_tokens:
            if "id" in token:
                token["id"] += token_id_offset

        all_tokens.extend(chunk_tokens)
        last = all_tokens[-1] if all_tokens else None
        token_id_offset = (last["id"] + 1) if last and "id" in last else token_id_offset + len(chunk_tokens)
...
def _compress_rle(tokens):
    ...
    for i in range(1, len(tokens)):
        if tokens[i]["text"] == current_token["text"] and tokens[i].get("type", "") == current_token.get("type", ""):
            count += 1
        else:
...
def _compress_pattern(tokens):
    ...
    for i in range(len(tokens) - 1):
        for pattern_len in range(2, min(5, len(tokens) - i + 1)):
            ...
            pattern_key = tuple((t["text"], t.get("type", "")) for t in pattern)
    ...
    for token in tokens:
        ...
        if key in token_map:
            compressed.append({
                ...
                # token_map deliberately NOT stored per-token (was O(n*v) memory bloat)
            })
        else:
...
def _reconstruct_byte_tokens(tokens):
    ...
    for char_index in sorted(char_groups.keys()):
        ...
        parts.append(_reconstruct_char_from_utf8_bytes(byte_values))
    return ''.join(parts)
...
# Thread-local flag: each thread/call gets its own value, preventing data races
# when parallel tokenizers call sanitize_text + compose_backend_number concurrently.
_RUN_COLLAPSE_TO_ONE = False  # fallback for environments without threading module


def _get_run_collapse():
    if _thread_local is not None:
        return getattr(_thread_local, 'run_collapse_to_one', False)
    return _RUN_COLLAPSE_TO_ONE


def _set_run_collapse(val):
    global _RUN_COLLAPSE_TO_ONE
    if _thread_local is not None:
        _thread_local.run_collapse_to_one = val
    else:
        _RUN_COLLAPSE_TO_ONE = val
...
def collapse_spaces(s):
    ...
    for ch in s:
        if _is_space(ch):
            ...
            out.append(ch)
            prev_space = False
    # Trim leading spaces only (preserve trailing space for accurate tokenization)
    start = 0
...
def remove_specials(s):
    return ''.join(ch for ch in s if _is_word_char(ch) or _is_space(ch))
...
def sanitize_text(s, use_lower, drop_specials, collapse_letters_to):
    ...
    if drop_specials:
        t = remove_specials(t)
    t = collapse_spaces(t)
    # If user asks for collapse to exactly 1, preserve text and enable run-aware math.
    # Use thread-local storage so concurrent calls don't corrupt each other.
    if collapse_letters_to == 1:
        _set_run_collapse(True)
    else:
        _set_run_collapse(False)
...
# Build the table immediately at module load — deterministic, no race condition,
# no dead assignment, no lazy-init lock needed.  Values: A/J/S=1, B/K/T=2, …
_ALPHABET_TABLE = [(i % 9) + 1 for i in range(26)]


def _init_alphabetic_table():
    """No-op — table is built at import time. Kept for backward-compat callers."""
...
def compose_backend_number(token_text, position_in_sentence, uid, neighbor_prev_uid, neighbor_next_uid, embedding_bit):
    # Choose weighted sum strategy (thread-local safe)
    if _get_run_collapse():
...
def _bytes_to_text_representation(raw_bytes):
    ...
    try:
        ...
        if len(decoded) > 0 and printable_count / len(decoded) > 0.7:  # 70% printable
            return decoded
    except Exception:
        pass
