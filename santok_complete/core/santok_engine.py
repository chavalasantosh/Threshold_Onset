...
class TextTokenizationEngine:
    ...
    def _normalize_case(self, text: str) -> str:
        ...
        # list + join is O(n) — avoids O(n²) string concatenation
        return ''.join(chr(ord(c) + 32) if 65 <= ord(c) <= 90 else c for c in text)
...
# Module-level singleton engine — avoids creating a new TextTokenizationEngine
# object on every tokenize_text() call (was re-running __init__ + _init_alphabetic_table
# on each invocation which is wasteful in batch-processing loops).
_DEFAULT_TOKENIZATION_ENGINE = TextTokenizationEngine()
...
def tokenize_text(
    ...
    return _DEFAULT_TOKENIZATION_ENGINE.tokenize(text, tokenization_method)
...
def analyze_text_comprehensive(text: str) -> Dict[str, Dict[str, Any]]:
    ...
    """
    return _DEFAULT_TOKENIZATION_ENGINE.analyze_text(text)
...
def generate_text_summary(text: str) -> Dict[str, Any]:
    ...
    Returns:
        Summary statistics
    """
    return _DEFAULT_TOKENIZATION_ENGINE.generate_summary(text)
