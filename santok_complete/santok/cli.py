...
def format_results(results: Dict[str, Dict[str, Any]], quiet: bool = False) -> str:
    ...
    for tokenization_method, result in results.items():
        ...
        if result['features']:
            ...
            output.append(f"  - Length Factor: {features.get('length_factor', 0)}")
            output.append(f"  - Balance Index: {features.get('balance_index', 0)}")
            output.append(f"  - Entropy Index: {features.get('entropy_index', 0)}")
            if not quiet and 'mean' in features:
                output.append(f"  - Mean: {features['mean']:.2f}")
                output.append(f"  - Variance: {features.get('variance', 0.0):.2f}")
