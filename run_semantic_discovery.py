...
Runs the full pipeline in-memory first to obtain Phase 2-4 outputs, then runs
Phases 5-9 (semantic discovery). No disk JSON required.

USAGE:
    python run_semantic_discovery.py

Env:
    SEMANTIC_LOG_PHASE10=1  — log one Phase 10 line after the pipeline (directed continuation).
"""
...
# Sample text used to run the pipeline when no Phase 2-4 data is on disk
DEFAULT_SAMPLE_TEXT = (
...
def _symbol_sequences_from_model_state(model_state):
    """Build symbol_sequences from pipeline model_state (residue_sequences + phase2/4)."""
...
def execute_complete_workflow():
    ...
    # ============================================================
    # STEP 1: Run full pipeline to get Phase 2-4 in memory
    # ============================================================
    logger.info("\n[STEP 1] Running full pipeline (Phases 0-4) in-memory...")
    try:
    ...
    if not phase4_output.get("identity_to_symbol"):
        logger.error(
            "Pipeline produced no Phase 4 identities (phase4_symbols empty). "
            "Use longer or richer input so Phase 2-4 yield persistent identities."
        )
        return
    ...
    try:
        ...
        logger.debug("Phase10 snapshot for semantic: %s", exc)
        phase10_snap = None

    if os.environ.get("SEMANTIC_LOG_PHASE10", "").lower() in ("1", "true", "yes"):
        if isinstance(phase10_snap, dict):
            logger.info(
                "[Phase10] gate_passed=%s total_transitions=%s universe_size=%s",
                phase10_snap.get("gate_passed"),
                phase10_snap.get("total_transitions"),
                len(phase10_snap.get("universe_ids") or []),
            )
        else:
            logger.info("[Phase10] no snapshot available for logging")
    ...
    try:
        constraint_engine = ConstraintDiscoveryEngine(
            ...
            config={
                "phase8": {
                    # Short demo corpora still discover n-grams when counts are low
