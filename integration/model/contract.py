...
def model_state_from_pipeline(state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Optional keys (ignored by the predict-next model API):
      - ``residue_sequences`` — list of residue runs (used by symbol extraction).
      - ``phase10_metrics`` — JSON blob when ``PipelineConfig.include_phase10_metrics``
        was True during ``run(..., return_model_state=True)``.
    """
