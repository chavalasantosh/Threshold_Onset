    ...
    try:
        ...
        if fn is not None:
            if method in ("subword", "subword_bpe", "subword_syllable", "subword_frequency"):
                ...
                tokens = fn(text)
            return _extract_token_strings(tokens)
    except Exception as exc:  # pylint: disable=broad-exception-caught
...
def run(
    ...
    *,
    ...
    Args:
        ...
        return_model_state: If True, result.model_state is set (phase2/3/4, path_scores, tokens, residue_sequences) for predict-next evaluation.
    ...
    if text_override is None:
        ...
        elif getattr(cfg, "_use_default_input", False):
            raw_input = (
                "Action before knowledge.\n"
...
async def async_run(
    ...
    return await loop.run_in_executor(
        None,
        functools.partial(run, text_override=text_override, cfg=cfg, learner=learner),
    )
...
if __name__ == "__main__":
    ...
    if text_override_cli is None and sys.stdin.isatty() and not getattr(cfg_cli, "_use_default_input", False):
        text_override_cli = _prompt_interactive_input().strip() or None
