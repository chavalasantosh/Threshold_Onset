...
- **Pipeline:** `run_complete.run(..., return_model_state=True)` returns `PipelineResult.model_state`. That dict is a valid `ModelState` for this contract. The model module does not call `run()`; the caller passes state in.
