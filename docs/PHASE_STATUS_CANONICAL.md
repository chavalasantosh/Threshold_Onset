...
**Directory map:** See [`CANONICAL_PHASE_MAP.md`](CANONICAL_PHASE_MAP.md) — semantic Phase 5 lives only under `threshold_onset/semantic/phase5/`; directed continuation is **Phase 10** at `threshold_onset/phase10/`.
...
## Extension: Phase 10 (structural metrics, not part of 0–4 freeze table)

**Phase 10** (`threshold_onset/phase10/`) is a **read-only analytics layer** on ordered identity streams: directed successor counts, empirical exclusion, and strict-dominance necessity. It does **not** replace or alter frozen Phases 0–4 and is **separate** from semantic Phases 5–9 (`threshold_onset/semantic/`).

- **Docs:** `threshold_onset/phase10/README.md`
- **Pipeline hook:** optional `phase10_metrics` on `model_state` when `include_phase10_metrics` / `PIPELINE_PHASE10_METRICS=1`
