...
# Optional: full pipeline state + Phase 10 blob (large)
result = process(
    "Your input text",
...
**Async pipeline:** `integration.run_complete.async_run(..., return_model_state=True)` mirrors sync `run()`.

**Phase 10 / identity-conditioned:** `threshold_onset.phase10`, `threshold_onset.identity_conditioned` — see `threshold_onset/phase10/README.md` and `docs/IDENTITY_CONDITIONED_CONTINUATION.md`.
