...
STEP 4 — TRAIN
  Reads data/hindu_corpus_real.jsonl (or --train-jsonl) and runs santek_base_model.py.
  Parallel En–Hi files: --train-jsonl data/ennhintrain.jsonl --jsonl-field en|hi|both
  Optional --test-jsonl data/ennhintest.jsonl (reported only; wire eval separately if needed).
  --epochs, --eta, --decay pass through.
...
def _record_to_text(rec: dict, jsonl_field: str) -> Optional[str]:
    """
    Map one JSON object to a training string.

    jsonl_field:
      text — hindu_corpus_real.jsonl style {"text": "..."}
      en, hi — En–Hi parallel lines {"en": "...", "hi": "..."}
      both — concatenate non-empty en and hi with a newline
    """
    if jsonl_field == "text":
        t = rec.get("text")
...
def load_jsonl_corpus(
    ...
    """Stream a JSONL file and collect training texts (parallel decode batches)."""
    path = path.resolve()
    if not path.is_file():
        ERR(f"Corpus file not found: {path}")
        sys.exit(1)

    field = jsonl_field.strip().lower()
    if field not in ("text", "en", "hi", "both"):
        ERR(f"Invalid --jsonl-field: {jsonl_field!r} (use text, en, hi, both)")
        sys.exit(1)
    ...
    # Cap early: scanning a multi-GB JSONL for only N rows must stop after N (sequential is fine).
    if max_texts is not None:
        ...
        workers = _corpus_io_workers()
        with path.open(encoding="utf-8", errors="replace") as f, concurrent.futures.ThreadPoolExecutor(
            max_workers=workers
        ) as pool:
            for line in f:
                batch.append(line)
    ...
    INFO(f"Loaded {len(texts):,} texts from {path.name} (field={field})")
    return texts
...
def load_existing_corpus(max_texts: Optional[int] = None) -> List[str]:
    ...
    if not CORPUS_FILE.exists():
        ...
        sys.exit(1)
    return load_jsonl_corpus(CORPUS_FILE, max_texts, "text")


def infer_jsonl_field(path: Path) -> str:
    """Peek first valid object: prefer ``text``, else ``en``/``hi`` parallel format."""
    path = path.resolve()
    try:
        with path.open(encoding="utf-8", errors="replace") as f:
            for line in f:
                ...
                if isinstance(t, str) and t.strip():
                    return "text"
                if isinstance(rec.get("en"), str) and rec["en"].strip():
                    return "en"
                if isinstance(rec.get("hi"), str) and rec["hi"].strip():
                    return "hi"
                break
    except OSError:
        pass
...
def resolve_data_path(p: Optional[Path]) -> Optional[Path]:
    """Resolve CLI path: absolute as-is; else ``<repo>/path`` then ``<repo>/data/path``."""
    if p is None:
        return None
    p = Path(p)
    if p.is_absolute():
...
def effective_jsonl_field(train_jsonl: Optional[Path], explicit: Optional[str]) -> str:
    if explicit is not None:
        return explicit.strip().lower()
    if train_jsonl is not None:
        return infer_jsonl_field(train_jsonl)
    return "text"
...
Examples:
  ...
  # En–Hindi JSONL (train file is loaded; test file is listed only — not used in training):
  python build_hindu_corpus.py --train-jsonl data/ennhintrain.jsonl --test-jsonl data/ennhintest.jsonl --jsonl-field en --epochs 18 --max-texts 2132
  ...
  python build_hindu_corpus.py --force-download
    ...
    ap.add_argument(
        ...
        metavar="PATH",
        help=(
            "Train from this JSONL (implies skip download). "
            "Use --jsonl-field en|hi|both|text for record shape."
        ),
    )
    ap.add_argument(
        "--test-jsonl",
        ...
        metavar="PATH",
        help=(
            "Companion held-out JSONL (same schema as --train-jsonl). "
            "Reported in the banner only; not fed to training. Point "
            "santek_base_model training_corpus_file_val / eval config at it if needed."
        ),
    )
    ap.add_argument(
        "--jsonl-field",
        type=str,
        default=None,
        choices=("text", "en", "hi", "both"),
        metavar="FIELD",
        help=(
            "Which field(s) to read per line (default: infer for --train-jsonl, else text). "
            "text=hindu corpus; en/hi/both=parallel En–Hi (id, en, hi)."
        ),
    )
    ...
    ap.add_argument("--check-corpus", action="store_true",
                    help="Print corpus stats; use with --train-jsonl to inspect that file (UTF-8 safe on Windows)")
...
def check_corpus_stats(
    ...
    if not path.exists():
        ERR(f"Corpus not found: {path}")
        sys.exit(1)
    try:
        size = path.stat().st_size
    except OSError as e:
        ERR(f"Cannot stat {path}: {e}")
        sys.exit(1)

    huge = size >= 80 * 1024 * 1024
    if huge:
        INFO(f"Large file ({size / (1024 * 1024):.0f} MiB) — one pass: line count + sample…")
        n_lines = 0
        texts: List[str] = []
        sample_cap = 50_000
        field = jsonl_field.strip().lower()
        with path.open(encoding="utf-8", errors="replace") as f:
            for line in f:
                ...
                try:
                    ...
                    continue
        print(f"  {n_lines:,} JSONL lines scanned")
        if not texts:
            WARN("No parsed texts in sample (check --jsonl-field).")
            return
        total_words = sum(len(t.split()) for t in texts)
        avg = total_words / len(texts)
        print(
            f"  Parsed sample: {len(texts):,} texts  |  {total_words:,} words  |  "
            f"avg {avg:.0f} words/text"
        )
        print(f"  Sample: {texts[0][:120]}{'...' if len(texts[0]) > 120 else ''}")
        return

    texts = load_jsonl_corpus(path, max_texts=None, jsonl_field=jsonl_field)
    if not texts:
...
def main():
    ...
    if getattr(args, "check_corpus", False):
        train_p = resolve_data_path(getattr(args, "train_jsonl", None))
        jf = effective_jsonl_field(train_p, args.jsonl_field)
        check_corpus_stats(corpus_path=train_p, jsonl_field=jf)
        return
    ...
    train_path = resolve_data_path(getattr(args, "train_jsonl", None))
    test_path = resolve_data_path(getattr(args, "test_jsonl", None))
    jfield = effective_jsonl_field(train_path, args.jsonl_field)

    print(f"\n  Data directory : {DATA_DIR}")
    if train_path:
        print(f"  Train JSONL    : {train_path}")
        print(f"  JSONL field    : {jfield}")
        if test_path:
            print(f"  Test JSONL     : {test_path}  (held out — not used in this training step)")
        print(f"  Sources        : (custom JSONL — download pipeline skipped)")
    else:
    ...
    if train_path:
        texts = load_jsonl_corpus(train_path, max_texts=args.max_texts, jsonl_field=jfield)
