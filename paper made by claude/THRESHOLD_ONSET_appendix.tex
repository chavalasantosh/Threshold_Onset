...
\textbf{Test suite.}
The test suite uses \texttt{pytest}. The HTTP server integration test (\texttt{test\_rest\_process}) is marked \texttt{@pytest.mark.slow} and uses a socket-polling startup check (up to 10 seconds) to avoid timing races. To run only the fast tests (88 tests, no server required):
\begin{center}
\texttt{pytest -m 'not slow'}
\end{center}
The \texttt{slow} marker is registered in \texttt{pyproject.toml} under \texttt{[tool.pytest.ini\_options]}. All 88 non-slow tests pass; the full suite is 89 tests, 88 passing.

\textbf{Semantic workflow scripts.}
\texttt{run\_semantic\_discovery.py} and \texttt{threshold\_onset/semantic/example\_complete\_workflow.py} run the full pipeline (Phases~0--4) in-memory first via \texttt{integration.run\_complete.run()}, then pass \texttt{phase4\_output} and symbol sequences directly to the Phase~5--9 engines. No disk JSON files are required.
